...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'polaris-suggest:createSeclogCall': () => this.createSeclogCall()
    }));
  },
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      this.download(urlxdoc + 'xdoc.php?search=' + selection).then((json) => {
        ...
        if(aFunctionsFounded.length == 1 || typeof aInfo[selection] == 'object') {
          ...
          sCallCreated = '$aParameters = array(' + "\n";
          for(var i = 0; i < aInfo[sProperty].parameters.length; i++) {
            sCallCreated += "\t" + '"' + aInfo[sProperty].parameters[i] + '" => "", '+"\n";
          }

          sCallCreated += ');'+"\n";
          sCallCreated += '$mRet = $oSecLog->execute_event($iFKUser, \'' + selection + '\', $aParameters, true);'+"\n";
          editor.insertText(sCallCreated)
        } else {
          atom.notifications.addWarning('Hay más de un evento que se corresponde con la busqueda <i>'+selection+'</i>.')
        }
      }).catch((error) => {
        console.log(error)
